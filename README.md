...
Click `CONNECT AI`, allow microphone access, and speak normally. The `ASK AI` button is still available as a manual cue, but ordinary presentation speech is transcribed without automatically creating an assistant response. The browser requests a spoken response only when the completed transcript contains a Crit A.I. wake-name mention, an Oura cue, or the configured human-versus-AI stage bit. `Critai` remains accepted as a wake-word spelling, but spoken responses should use `Crit A.I.` so it is pronounced "crit ay eye".

The controller starts in pre-turn mode, so anything before the repeated "AI AI AI" intro-video marker or Keaton self-introduction cannot trigger Crit A.I. A self-introduction such as "I'm Keaton", "I am Keaton", "my name is Keaton", or the likely "I'm kitten" transcription miss arms Crit A.I. without making it speak. When the AI-supercut marker appears, the controller deletes pre-marker context and the video item, clears the input buffer only if nobody is actively speaking, arms Crit A.I. for Keaton's live turn, and replays any post-video cue that transcribed before the long video transcript finished.

The session uses `semantic_vad` with `eagerness: high` and `create_response: false`, plus `audio.input.noise_reduction: { type: "far_field" }` so coughs, room noise, and mic bleed are less likely to trigger false turns. While Crit A.I. is speaking, the browser also gates the outbound mic track until the WebRTC output audio buffer has drained, preventing speaker bleed from being sent back to the model. Finale mode switches to a faster silence-based VAD, also with automatic responses disabled, so the browser can keep applying the same transcript gate. Response interruption stays disabled, spoken responses use the model's full available output budget to avoid app-level mid-sentence cutoffs, and rolling conversation context is limited with retention-ratio truncation to reduce long-session slowdown.

Crit A.I. is prompted to keep normal replies short, substantive, and stage-ready rather than being forced short with a hard output cap.

Because `gpt-realtime-2` can produce both `commentary` and `final_answer` audio phases in one response, Crit A.I. is also prompted to keep normal turns to a single spoken `final_answer` phase. This avoids a direct cue being heard as a preamble plus a second final response on the WebRTC audio stream.
